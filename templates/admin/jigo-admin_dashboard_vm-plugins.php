...
if( $wp_plugins && $vl_plugins ) {
	...
	if( isset( $wp_plugins_update ) ) {
		$message = __( 'A new version of a Visser Labs Plugin for WP e-Commerce is available for download.', 'jigo_vm' );
		$output .= '<p class="message">' . $message . '</p>';
	}

	$output .= '<table class="jigo_vm_version_table">';
	$output .= '<tr><th style="text-align:left;">' . __( 'Plugin', 'jigo_vm' ) . '</th><th style="text-align:left;">' . __( 'Version', 'jigo_vm' ) . '</th><th style="text-align:left;">' . __( 'Status', 'jigo_vm' ) . '</th></tr>';
	...
	$message = __( 'Looking for more WP e-Commerce Plugins?', 'jigo_vm' );
	...
	$message = __( 'Connection failed. Please check your network settings.', 'jigo_wm' );
	$output .= '<p>' . $message . '</p>';
